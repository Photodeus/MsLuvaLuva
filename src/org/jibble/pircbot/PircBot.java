...
public abstract class PircBot implements ReplyConstants {
	...
	private final Map<Channel, Set<User>> channel_users = new Hashtable<Channel, Set<User>>();
	...
	private String _finger = "Response disabled";
	...
	protected void handleLine(String line) {
		...
		if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0 && line.endsWith("\u0001")) {
			...
			this.addUser(channel, new User(User.Prefix.NONE, sourceNick, sourceLogin, sourceHostname));
			this.onJoin(target, sourceNick, sourceLogin, sourceHostname);
		} else if (command.equals("PART")) {
			// Someone is parting from a channel.
			this.removeUser(new Channel(target), sourceNick);
			if (sourceNick.equals(this.getNick())) {
				this.removeChannel(new Channel(target));
			}
			...
			this.removeUser(new Channel(target), recipient);
	...
	private final void processServerResponse(int code, String response) {
		...
		if (code == RPL_LIST) {
			...
			while (tokenizer.hasMoreTokens()) {
				...
				User.Prefix prefix = User.Prefix.NONE;
				if (nick.startsWith("@")) {
					prefix = User.Prefix.OP;
				} else if (nick.startsWith("%")) {
					prefix = User.Prefix.HALFOP;
				} else if (nick.startsWith("+")) {
					prefix = User.Prefix.VOICE;
				} else if (nick.startsWith("&")) {
					prefix = User.Prefix.ADMIN;
				} else if (nick.startsWith("~")) {
					prefix = User.Prefix.OWNER;
				}
				nick = nick.replaceAll("[&~+%@]", "");
				this.addUser(new Channel(channel), new User(prefix, nick));
			}
	...
	protected void onPart(String channel, String sender, String login, String hostname) {
		this.onPart(channel, sender, login, hostname, null);
	}
	/**
	 * This method is called whenever someone (possibly us) parts a channel
	 * which we are on.
	 ...
	 * @param reason
	 */
	protected void onPart(String channel, String sender, String login, String hostname, final String reason) {
	}
		...
		if (_channelPrefixes.indexOf(target.charAt(0)) >= 0) {
			...
			for (int i = 0; i < params[0].length(); i++) {
				...
				if (atPos == '+' || atPos == '-') {
					...
					if (pn == '+') {
						this.updateUser(channel, UserRightAction.OP_ADD, params[p]);
						onOp(target, sourceNick, sourceLogin, sourceHostname, params[p]);
					} else {
						this.updateUser(channel, UserRightAction.OP_REMOVE, params[p]);
						onDeop(target, sourceNick, sourceLogin, sourceHostname, params[p]);
					}
					p++;
				} else if (atPos == 'v') {
					if (pn == '+') {
						this.updateUser(channel, UserRightAction.VOICE_ADD, params[p]);
						onVoice(target, sourceNick, sourceLogin, sourceHostname, params[p]);
					} else {
						this.updateUser(channel, UserRightAction.VOICE_REMOVE, params[p]);
						onDeVoice(target, sourceNick, sourceLogin, sourceHostname, params[p]);
					}
					p++;
				} else if (atPos == 'q') {
					if (pn == '+') {
						this.updateUser(channel, UserRightAction.OWNER_ADD, params[p]);
						onOwner(target, sourceNick, sourceLogin, sourceHostname, params[p]);
					} else {
						this.updateUser(channel, UserRightAction.OWNER_REMOVE, params[p]);
						onDeOwner(target, sourceNick, sourceLogin, sourceHostname, params[p]);
					}
					p++;
				} else if (atPos == 'a') {
					if (pn == '+') {
						this.updateUser(channel, UserRightAction.ADMIN_ADD, params[p]);
						onAdmin(target, sourceNick, sourceLogin, sourceHostname, params[p]);
					} else {
						this.updateUser(channel, UserRightAction.ADMIN_REMOVE, params[p]);
						onDeAdmin(target, sourceNick, sourceLogin, sourceHostname, params[p]);
					}
					p++;
				} else if (atPos == 'h') {
					if (pn == '+') {
						this.updateUser(channel, UserRightAction.HALFOP_ADD, params[p]);
						onHalfOp(target, sourceNick, sourceLogin, sourceHostname, params[p]);
					} else {
						this.updateUser(channel, UserRightAction.HALFOP_REMOVE, params[p]);
	...
	protected void onDeOwner(String channel, String sourceNick, String sourceLogin, String sourceHostname, String recipient) {
	}
	protected void onOwner(String channel, String sourceNick, String sourceLogin, String sourceHostname, String recipient) {
	}
	protected void onDeAdmin(String channel, String sourceNick, String sourceLogin, String sourceHostname, String recipient) {
	}
	protected void onAdmin(String channel, String sourceNick, String sourceLogin, String sourceHostname, String recipient) {
	}
	...
	public final String[] getChannels() {
		String[] ca;
		synchronized (channel_users) {
			ca = new String[channel_users.size()];
			Iterator<Channel> it = channel_users.keySet().iterator();
			for (int i = 0; i < ca.length; i++) {
				ca[i] = it.next().channel;
			}
		}
		return ca;
	}
	 ...
	 */
	private final void addUser(Channel channel, User user) {
		synchronized (channel_users) {
			Set<User> users = channel_users.get(channel);
			if (users == null) {
				users = new HashSet<User>();
			}
			users.add(user);
			channel_users.put(channel, users);
		}
	}
	 ...
	 */
	private User removeUser(Channel channel, String nick) {
		User user = new User(User.Prefix.NONE, nick);
		synchronized (channel_users) {
			Set<User> users = channel_users.get(channel);
			if (users != null) {
	...
	private void removeUser(String nick) {
		synchronized (channel_users) {
			for (Channel channel : channel_users.keySet()) {
	...
	private void renameUser(String oldNick, String newNick) {
		synchronized (channel_users) {
			for (Channel channel : channel_users.keySet()) {
				User found = this.removeUser(channel, oldNick);
				if (found != null) {
					User newuser = new User(found.getPrefix(), newNick, found.getLogin(), found.getHostname());
					this.addUser(channel, newuser);
				}
			}
		}
	}
	...
	private void updateUser(Channel channel, UserRightAction userMode, String nick) {
		synchronized (channel_users) {
			Set<User> users = channel_users.get(channel);
			if (users != null) {
				for (User usr : users) {
					if (usr.equals(nick)) {
						switch (userMode) {
						case OWNER_ADD:
							usr.addPrefix(User.Prefix.OWNER);
							break;
						case OWNER_REMOVE:
							usr.removePrefix(User.Prefix.OWNER);
							break;
						case ADMIN_ADD:
							usr.addPrefix(User.Prefix.ADMIN);
							break;
						case ADMIN_REMOVE:
							usr.removePrefix(User.Prefix.ADMIN);
							break;
						case OP_ADD:
							usr.addPrefix(User.Prefix.OP);
							break;
						case OP_REMOVE:
							usr.removePrefix(User.Prefix.OP);
							break;
						case HALFOP_ADD:
							usr.addPrefix(User.Prefix.HALFOP);
							break;
						case HALFOP_REMOVE:
							usr.removePrefix(User.Prefix.HALFOP);
							break;
						case VOICE_ADD:
							usr.addPrefix(User.Prefix.VOICE);
							break;
						case VOICE_REMOVE:
							usr.removePrefix(User.Prefix.VOICE);
							break;
						}
						//users.remove(usr);
						//users.add(usr);
	...
	class Channel {
		...
		@Override
		public int hashCode() {
			return channel.toLowerCase().hashCode();
		}
		...
		public boolean equals(final Object obj) {
			if (obj instanceof Channel) {
				...
				return channel.equalsIgnoreCase(ch.channel);
			}
			return false;
		}
	}
